...
EVP_MAC_CTX* ByteCrypto::hmac_sha1_ctx = nullptr;
...
void ByteCrypto::Init() {
    ...
    if (mac) {
        ByteCrypto::hmac_sha1_ctx = EVP_MAC_CTX_new(mac);
        EVP_MAC_free(mac);
    }
...
void ByteCrypto::DeInit() {
    ...
    if (ByteCrypto::hmac_sha1_ctx) {
        EVP_MAC_CTX_free(ByteCrypto::hmac_sha1_ctx);
...
uint8_t* ByteCrypto::GetHmacSha1(const std::string& key, const uint8_t* data, size_t len) {
    if (!ByteCrypto::hmac_sha1_ctx) {
        throw CppStreamException("HMAC context not initialized");
    }
    ...
    int ret = EVP_MAC_init(ByteCrypto::hmac_sha1_ctx, 
                           (const unsigned char*)key.c_str(), 
                           key.length(), 
                           params);
    if (ret != 1) {
        throw CppStreamException("OpenSSL EVP_MAC_init() failed with key");
    }

    ret = EVP_MAC_update(ByteCrypto::hmac_sha1_ctx, data, len);
    if (ret != 1) {
        throw CppStreamException("OpenSSL EVP_MAC_update() failed");
    }

    size_t ret_len = 0;
    ret = EVP_MAC_final(ByteCrypto::hmac_sha1_ctx, 
                        (uint8_t*)ByteCrypto::hmac_sha1_buffer, 
                        &ret_len, 
                        sizeof(ByteCrypto::hmac_sha1_buffer));
    if (ret != 1) {
        throw CppStreamException("OpenSSL EVP_MAC_final error");
    }
    if (ret_len != 20) {
        throw CppStreamException("OpenSSL EVP_MAC_final returned wrong length");
    }
